...
import { AngleDamper } from '@/utils/math/angleDamper'
...
class FlyControls extends Controls {
  ...
  public set options (value: FlyControlsOptions) {
    ...
    if (value.moveSpeed) {
      console.warn('Fly speed modifier: ',
        this._options.moveSpeed)
    }
    this.setDamperDecayTime(this._options.damperDecay)
  }

  public set targetCamera (target: PerspectiveCamera) {
  ...
  public update (delta?: number): boolean {
    ...
    if (this.keyMap.forward) {
      this.velocity.z = -scaledWalkingSpeed * this._options.moveSpeed *
        deltaSeconds
    }
    if (this.keyMap.back) {
      this.velocity.z = scaledWalkingSpeed * this._options.moveSpeed *
    ...
    if (!this.keyMap.forward && !this.keyMap.back) {
      this.velocity.z = 0
    }

    if (this.keyMap.left) {
      this.velocity.x = -scaledWalkingSpeed * this._options.moveSpeed *
        deltaSeconds
    }
    ...
    if (this.keyMap.up) {
      this.velocity.y = scaledWalkingSpeed * this._options.moveSpeed *
        deltaSeconds
    }
    if (this.keyMap.down) {
      this.velocity.y = -scaledWalkingSpeed * this._options.moveSpeed *
    ...
    if (!this.keyMap.down && !this.keyMap.up) {
      this.velocity.y = 0
    }
  ...
  protected updatePositionRotation (delta: number) {
    ...
    const minMaxRange = diagonal < 1
      ? this.world.getRelativeOffset(diagonal)
      : 1
    ...
    this.euler.x = this.eulerXDamper.update(this.euler.x, this.goalEuler.x,
      delta, 1)
    this.euler.y = this.eulerYDamper.update(this.euler.y, this.goalEuler.y,
      delta, 1)
    this.euler.z = this.eulerZDamper.update(this.euler.z, this.goalEuler.z,
      delta, 1)
  ...
  public fitToSphere (sphere: Sphere): void {
    ...
    forward.negate()
    const pos = new Vector3().copy(sphere.center).
  ...
  public fromPositionAndTarget (position: Vector3, target: Vector3): void {
    ...
    if (cPos.equals(position) && cTarget.equals(target)) {
      return
    }

    const tPosition = new Vector3().copy(position)
    const tTarget = new Vector3().copy(target)
    const matrix = new Matrix4().lookAt(tPosition, tTarget, this._up).
  ...
  protected disconnect () {
    ...
    for (const k in this.keyMap) {
      this.keyMap[k as MoveType] = false
    }
  }
  ...
  protected onMouseMove = (event: PointerEvent) => {
    if (event.buttons !== 1 || !this._enabled) {
      return
    }
