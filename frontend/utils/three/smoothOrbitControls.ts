...
import { Controls } from '@/utils/three/controls'
import { Damper, SETTLING_TIME } from '@/utils/math/damper'
 ...
 */
export class SmoothOrbitControls extends Controls {
  ...
  protected _targetCamera: PerspectiveCamera
  ...
  constructor (
    ...
    const billboardMaterial = new MeshBasicMaterial({ color: 0x047efb })
    ...
    this.orbitSphere = new Mesh(new SphereGeometry(0.5, 32, 16),
      billboardMaterial)
    this.orbitSphere.visible = false
  ...
  set targetCamera (value: PerspectiveCamera) {
  ...
  public fromPositionAndTarget (position: Vector3, target: Vector3): void {
    ...
    const epsilon = 1e-6
    if (deltaPosition < epsilon && deltaTarget < epsilon) {
      return
    }
  ...
  public fitToSphere (sphere: Sphere) {
    ...
    const nativeOrigin = new Vector3().copy(sphere.center).
  ...
  public setOrbit (
    ...
    const nextTheta = clamp(goalTheta, minimumAzimuthalAngle,
      maximumAzimuthalAngle)
  ...
  adjustOrbit (deltaTheta: number, deltaPhi: number, deltaZoom: number) {
    ...
    const normalizedRadius =
      ...
      this.world.worldBox.getSize(new Vector3()).length()
    ...
    if (this._options.zoomToCursor) {
      ...
      if (goalRadius < this._options.minimumRadius &&
        this._options.infiniteZoom) {
      ...
      const cameraDirection = new Vector3().setFromSpherical(this.spherical).
        normalize().
        negate()
      const planeX = new Vector3().copy(cameraDirection).
      ...
      const worldToScreen =
        clamp(this.goalSpherical.radius, Math.abs(zoomAmount),
          Number.MAX_VALUE) *
        Math.tan(Math.exp(this.logFov) * MathUtils.DEG2RAD * 0.5)
      const cursor = new Vector3().copy(this.goalOrigin).add(
        planeX.multiplyScalar(
          this.zoomControlCoord.x * worldToScreen * aspect * +tasIntersect
        )
      ).add(
        planeY.multiplyScalar(
          this.zoomControlCoord.y * worldToScreen * +tasIntersect)
      ).add(dollyAmount)
      const lerpRatio = clamp(this._radiusDelta / this.goalSpherical.radius, -1,
        1)
      const newTargetEnd = new Vector3().copy(this.goalOrigin).
  ...
  public update (delta?: number): boolean {
    ...
    const minMaxRange = this._options.maximumRadius -
      this._options.minimumRadius
    ...
    let normalization = 1
    if (this.world) {
      normalization = this.world.worldBox.getBoundingSphere(
  ...
  protected polarFromPivotal (position: Vector3) {
    ...
    const pivotPoint = new Vector3().copy(this.pivotPoint).
      applyMatrix4(this._basisTransformInv)
    ...
    const polarOrigin = camPos.sub(
      new Vector3().copy(dir).multiplyScalar(polarRadius))
  ...
  protected moveCamera (): boolean {
    ...
    if (this.usePivotal) {
      ...
      const pivotPoint = new Vector3().copy(this.pivotPoint).
        applyMatrix4(this._basisTransformInv)
      const prevPivotPoint = new Vector3().copy(this.lastPivotPoint).
        applyMatrix4(this._basisTransformInv)
      ...
      if (deltaPivot.length() > 0) {
        this.pivotalOrigin.copy(
          this.getPivotalOrigin(pivotPoint, position, quaternion))
      }
  ...
  protected touchModeZoom: TouchMode = (dx: number, dy: number) => {
    const touchDistance = this.twoTouchDistance(this.pointers[0],
      this.pointers[1])
    const deltaZoom =
      (ZOOM_SENSITIVITY *
        this._options.zoomSensitivity *
  ...
  protected initializePan () {
    ...
    this.panPerPixel =
      (PAN_SENSITIVITY * this._options.panSensitivity *
        +this._options.enablePan) /
      this._container.offsetHeight
  ...
  protected movePan (dx: number, dy: number) {
    const dxy = vector3.set(dx, dy, 0).
      multiplyScalar(this._options.inputSensitivity)
    ...
    const metersPerPixel =
      Math.max(relativeFactor, radiusFactor) * Math.exp(this.logFov) *
      this.panPerPixel
  ...
  protected filterOrbitToCursorHits () {
    return true
  }
  ...
  protected onTouchChange (event: PointerEvent) {
    if (this.pointers.length === 1) {
      ...
      this.lastSeparation = this.twoTouchDistance(this.pointers[0],
        this.pointers[1])
