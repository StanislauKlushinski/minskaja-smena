...
OBB.prototype.equals = function (other: OBB, epsion = 1e-6) {
  ...
  for (let i = 0; i < 9; i++) {
    if (Math.abs(this.rotation.elements[i] - other.rotation.elements[i]) >
      epsion) {
      return false
    }
  }
...
Box3.prototype.intersectOBB = function (obb: OBB): OBB | null {
  ...
  for (const pt of intersectionPoints) {
    for (let i = 0; i < 3; i++) {
      ...
      halfSize.setComponent(i,
        Math.max(halfSize.getComponent(i), Math.abs(proj)))
    }
  }
